...
void GameScene::glLineWidth(GLfloat width)
{
#ifdef __APPLE__
    ::glLineWidth(1);
#else
...
void GameScene::InitResources()
{
    ...
    objectModel = new gfxc::Transform();

    camera = new gfxc::Camera();
    //camera->SetPerspective(60, window->props.aspectRatio, 0.01f, 200);
    camera->SetOrthographic(0, 10, 0, 10, 0.01, 200);
    camera->m_transform->SetMoveSpeed(2);
    camera->m_transform->SetWorldPosition(glm::vec3(0, 0, 7));
    camera->m_transform->SetWorldRotation(glm::vec3(0, 0, 0));
    camera->Update();

    cameraInput = new gfxc::CameraInput(camera);
    window = Engine::GetWindow();

    GameSceneInput *SI = new GameSceneInput(this);
    (void)SI;
...
void GameScene::AddMeshToList(Mesh * mesh)
{
    if (mesh->GetMeshID())
    {
...
void GameScene::DrawCoordinateSystem()
{
    DrawCoordinateSystem(camera->GetViewMatrix(), camera->GetProjectionMatrix());
}


void GameScene::DrawCoordinateSystem(const glm::mat4 & viewMatrix, const glm::mat4 & projectionMaxtix)
{
    glLineWidth(1);
...
void GameScene::RenderMesh(Mesh * mesh, Shader * shader, glm::vec3 position, glm::vec3 scale)
{
...
void GameScene::RenderMesh(Mesh * mesh, glm::vec3 position, glm::vec3 scale)
{
    RenderMesh(mesh, shaders["Simple"], position, scale);
}


void GameScene::RenderMesh2D(Mesh * mesh, Shader * shader, const glm::mat3 &modelMatrix)
{
    if (!mesh || !shader || !shader->program)
        return;
...
void GameScene::RenderMesh2D(Mesh * mesh, const glm::mat3 & modelMatrix, const glm::vec3 & color) const
{
...
void GameScene::RenderMesh(Mesh * mesh, Shader * shader, const glm::mat4 & modelMatrix)
{
    if (!mesh || !shader || !shader->program)
        return;
...
void GameScene::ReloadShaders() const
{
    std::cout << std::endl;
    std::cout << "=============================" << std::endl;
...
gfxc::Camera * GameScene::GetSceneCamera() const
{
    return camera;
}


InputController * GameScene::GetCameraInput() const
{
    return cameraInput;
}


void GameScene::ClearScreen(const glm::vec3 &color)
{
...
bool GameScene::ToggleGroundPlane()
{
