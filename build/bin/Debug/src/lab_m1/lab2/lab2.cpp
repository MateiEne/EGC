...
void Lab2::Init()
{
	...
	vector<VertexFormat> tetrahedronVerticies{
		// position, color, norm
		...
		VertexFormat(glm::vec3(0.5f, 0, 1), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(0.5f, 1, 0.5f), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
	};

	vector<unsigned int> tetrahedronIndicies = {
		0, 1, 2,
		0, 2, 3,
		2, 1, 3,
		0, 3, 1,
	...
	vector<VertexFormat> squareVerticies{
		VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.5, 0, 0.5), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0.5, 0, 0.5), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(1, 1, 0), glm::vec3(0.5, 0, 0.5), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0.5, 0, 0.5), glm::vec3(0.2, 0.8, 0.6)),
	};

	vector<unsigned int> squareIndicies = {
		0, 2, 3,
		0, 2, 1,
	};

	CreateMesh("tetraedru", tetrahedronVerticies, tetrahedronIndicies);
	// TODO(student): Create a square using two triangles with
	// opposing vertex orientations.
	CreateMesh("patrat", squareVerticies, squareIndicies);

	// bonus
	vector<VertexFormat> coloredTetrahedronVerticies{
		// position, color, norm
		VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(1, 0, 0), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(0.5f, 0, 1), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
		VertexFormat(glm::vec3(0.5f, 1, 0.5f), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
	};

	vector<unsigned int> coloredTetrahedronIndicies = {
		0, 1, 2,
		0, 2, 3,
		0, 3, 1,
		1, 2, 3,
	...
	CreateMesh("teatraedru_colorat", coloredTetrahedronVerticies, coloredTetrahedronIndicies);
}
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetraedru"], shaders["VertexColor"], glm::vec3(1, 1, 0), glm::vec3(0.25f));

	// TODO(student): Draw the square
	RenderMesh(meshes["patrat"], shaders["VertexColor"], glm::vec3(-1, 1, 0), glm::vec3(0.25f));

	// bonus
	RenderMesh(meshes["teatraedru_colorat"], shaders["VertexColor"], glm::vec3(0, 2, 0), glm::vec3(0.25f));
